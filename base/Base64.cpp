...
std::string base64Decode(const std::string& src)
{
    ...
    auto origin_bit_len = (src.size()-n) * 6 - padding_bit_len; // 48-12-4 = 32.
    ...
    for (auto ch : src) {
        ...
        if (detail::isBase64(ch)) {
            binary_str += std::bitset<6>(detail::base64_digits[static_cast<unsigned int>(ch)]).to_string();
        } else {
    ...
    for (size_t i = 0; i < origin_bit_len / 8; ++i) {
        dst += static_cast<char>(std::stoi(std::string(result, i*8, 8), nullptr, 2));
    }
